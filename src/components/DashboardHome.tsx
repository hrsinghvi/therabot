...
const DashboardHome = () => {
  ...
  return <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8">
      {/* Welcome Section */}
      <motion.div variants={itemVariants} className="text-center">
        <motion.h2 className="text-4xl font-light text-foreground mb-4" animate={{
        opacity: [0.7, 1, 0.7]
      }} transition={{
        duration: 3,
        repeat: Infinity
        ...
        <Card className="border-0 bg-gradient-to-r from-primary/10 to-accent/10 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="space-y-3">
              {weeklyMoodData.slice(0, 4).map((day, index) => <motion.div key={day.day} initial={{
              opacity: 0,
              x: -20
            }} animate={{
              opacity: 1,
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1 bg-muted rounded-full h-1.5">
                        <div className={`h-1.5 rounded-full transition-all duration-500 ${getIntensityColor(day.intensity)}`} style={{
                      width: `${day.intensity * 10}%`
                    }} />
                      </div>
      ...
      <motion.div variants={itemVariants}>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {quickActions.map((action, index) => <motion.div key={action.label} whileHover={{
          scale: 1.03,
          y: -2
        }} whileTap={{
          scale: 0.97
